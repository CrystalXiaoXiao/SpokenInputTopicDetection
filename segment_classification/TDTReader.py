    ...
    def read_bnd_sample_files(self, boundary_path):
        ...
        for name in os.listdir(boundary_path):
            if MAX_SAMPLES >= 1000000:
                break
            if "_MAN" not in name and (".fdch" not in name):
...
def dump_story_segments(segments, corpus):
    num = 0
    story_dict = dict()
    story_topics = dict()

    for key, stories in segments.iteritems():
        text = corpus[key]

        for story in stories:
            #story = Segment(story)
            if story.topic_annot != -1 and len(story.topic_annot) > 0:
                segment = text[story.beg_id - 1 : story.end_id]
                if story.id in story_dict:
                    print "ERROR - Duplicate Story with id: " + story.id
                    ...
                    story_dict[story.id] = sentences
                    story_topics[story.id] = story.topic_annot
    pickle.dump(story_dict, open('./story_segments.pckl', 'w'))
    pickle.dump(story_topics, open('./story_topics.pckl', 'w'))
...
def main_process(load_from_disk):
    ...
    if not load_from_disk:
        ...
        print("dumping story segments...")
        dump_story_segments(corpus.doc_boundaries, corpus.text_corpus)
        return 0
