...
class TrainableEmbeddings:
    ...
    def train_word2vec_model(self, data_path):
        print data_path
        #model parameters#
        dim = int(CONFIG["WORD2VEC"]["dim"])
        context = int(CONFIG["WORD2VEC"]["context"])
        min_count = int(CONFIG["WORD2VEC"]["min_count"])
        workers = int(CONFIG["WORD2VEC"]["workers"])
        sg = int(CONFIG["WORD2VEC"]["skipgram"])
        ...
        word2vec_model.save_word2vec_format('./w2v_ger_%id_%icontext_%s.txt'%(dim,context,model_str),binary=False)
